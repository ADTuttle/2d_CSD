...
//const PetscReal   dy = 0.02;        //grid size in y direction (in cm)
    const PetscReal  Lx = Nx*dx;          //width of domain in cm (x)
    const PetscReal  Ly = Ny*dy;         //length of domain in cm (y)
    const PetscInt  Nt = Time/dt;     //total number of time steps

    const PetscReal numrecords = Time/(dt*krecordfreq);
...
//const PetscInt  Nv = (Ni+1)*Nc; //version if volume is excluded
    const PetscInt  Nv = ((Ni+2)*Nc-1)*(!separate_vol)+((Ni+1)*Nc)*separate_vol;  //combining the above two with the separate_vol
    const PetscInt  NA = Nx*Ny*Nv;     //total number of unknowns
    const PetscInt Nz = Ni*Nc*(4*(Nx-1)*Ny+4*(Ny-1)*Nx+2*Nx*Ny)+Ni*(Nc-1)*6*Nx*Ny+(Nc*Ni+1)*Nx*Ny+(Nc-1)*(6*Nx*Ny+Nx*Ny*(Nc-2)+Ni*2*Nx*Ny);

    const PetscInt  itermax = 10;      //maximum Newton iterations allowed
//const PetscInt itermax = 4;
    const PetscReal  reltol = 1e-11;    //relative tolerance
...
//physical constants
    ...
    const  PetscReal  RTFC = R*T/FC;
//So we define pi here.

//Bath variables
    const PetscReal cbath[3]={140*1e-3,3.4*1e-3,120*1e-3}; //Na, K, and Cl
    const PetscReal Batheps=1; //Bath diffusion multiplier
    const PetscReal phibath=-0/RTFC;

//excitation parameters
    const  PetscReal  pmax  = 5;          //max value for excitation
//const  PetscReal  pmax  = 25;          //max value for excitation
    const  PetscReal  texct = 2;          //time for excitation
    const  PetscInt  Nexct = 5;          //number of grid points for excitation in each direction
...
//permeabilities in cm/s from Kager, 2000 and Yao, Huang, Miura, 2011.
    const PetscReal pNaT = 0;                  //1e-4%0%1e-3%if set to 0, recovery possible
    const PetscReal pNaP = 2e-5;
    const PetscReal pKDR = 1e-3;
    const PetscReal pKA = 1e-4;

//Leak conductances in mS/cm^2 from Kager, 2000 or Yao, Huang, Miura, 2011.
    const PetscReal pKLeak = 7e-2*RTFC/FC;     //Kager:10e-2,Miura:7e-2%K Leak conductance in mS/cm^2 converted to mmol/cm^2/s
    const PetscReal pClLeak = 10e-2*RTFC/FC;   //Kager:10e-2,Miura:20e-2%Cl Leak conductance in mS/cm^2 converted to mmol/cm^2/s
    const PetscReal pClLeakg = 5e-2*RTFC/FC;

//Glial KIR from Steinberg et al 2005
    const PetscReal pKIR = .13*RTFC/FC;        //conductance in mS/cm^2 converted to mmol/cm^2/s
    const PetscReal pKLeakadjust = 1.0;        //parameter for varying glial permeability

//pump current, parameters from Yao, Huang, Miura, 2011
    const PetscReal mK = 2e-3;                 //pump current constant in mmol/cm^3=mol/l
    const PetscReal mNa = 7.7e-3;              //pump current constant in mmol/cm^3=mol/l
    const PetscReal glpump = 1.0;              //multiplier to change glial pump rate
    const PetscReal npump = 1.0;
