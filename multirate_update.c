...
PetscErrorCode calc_residual_slow(SNES snes,Vec current_state,Vec Res,void *ctx)
{
    ...
    ierr = VecGetArray(user->state_vars->v_fast,&user->state_vars->phi_fast);CHKERRQ(ierr);
            ...
            for(ion=0;ion<Ni;ion++) {
                for(comp=0;comp<Nc-1;comp++) {
                    ...
                    if(x>0) {
                        ...
                        Rcvx = Rcvx*(log(c[c_index(x,y,comp,ion,Nx)])-log(c[c_index(x-1,y,comp,ion,Nx)])+z[ion]*(phi[phi_index(x,y,comp,Nx)]+phifast[phi_index(x,y,comp,Nx)]-phi[phi_index(x-1,y,comp,Nx)]-phifast[phi_index(x-1,y,comp,Nx)]))/dx*dt/dx;
                    }
                    ...
                    if(x<Nx-1) {
                        ...
                        RcvxRight = RcvxRight*(log(c[c_index(x+1,y,comp,ion,Nx)])-log(c[c_index(x,y,comp,ion,Nx)])+z[ion]*(phi[phi_index(x+1,y,comp,Nx)]+phifast[phi_index(x+1,y,comp,Nx)]-phi[phi_index(x,y,comp,Nx)]-phifast[phi_index(x,y,comp,Nx)]))/dx*dt/dx;
                    }
                    Rcvy = 0;
                    RcvyUp = 0;
                    //Up down difference
                    if(y>0) {
                        ...
                        Rcvy = Rcvy*(log(c[c_index(x,y,comp,ion,Nx)])-log(c[c_index(x,y-1,comp,ion,Nx)])+z[ion]*(phi[phi_index(x,y,comp,Nx)]+phifast[phi_index(x,y,comp,Nx)]-phi[phi_index(x,y-1,comp,Nx)]-phifast[phi_index(x,y-1,comp,Nx)]))/dy*dt/dy;
                    }
                    //Next upward difference
                    if(y<Ny-1) {
                        RcvyUp = Dcs[c_index(x,y,comp,ion,Nx)*2+1]*(cp[c_index(x,y,comp,ion,Nx)]+cp[c_index(x,y+1,comp,ion,Nx)])/2;
                        RcvyUp = RcvyUp*(log(c[c_index(x,y+1,comp,ion,Nx)])-log(c[c_index(x,y,comp,ion,Nx)])+z[ion]*(phi[phi_index(x,y+1,comp,Nx)]+phifast[phi_index(x,y+1,comp,Nx)]-phi[phi_index(x,y,comp,Nx)]-phifast[phi_index(x,y,comp,Nx)]))/dy*dt/dy;
                    }
    ...
    ierr = VecRestoreArray(user->state_vars->v_fast,&user->state_vars->phi_fast);
    if(Profiling_on) {
...
PetscErrorCode calc_residual_fast(SNES snes,Vec current_state,Vec Res,void *ctx)
{
    ...
    ierr = extract_subarray(user->state_vars->v,user->state_vars); CHKERRQ(ierr);
    ierr = VecGetArray(current_state,&user->state_vars->phi_fast); CHKERRQ(ierr);
    //Compute membrane ionic flux relation quantitites
    ionmflux(user);
    ...
    for(x=0;x<Nx;x++) {
        for(y=0;y<Ny;y++) {
            ...
            for(comp=0;comp<Nc-1;comp++) {
                ...
                //Add the terms shared with extracell
                ResphN -= Resph; // Subtract total capacitance, subtract total ion channel flux
                ierr = VecSetValue(Res,phi_index(x,y,comp,Nx),Resph,INSERT_VALUES); CHKERRQ(ierr);
            }
            ...
            for(ion=0;ion<Ni;ion++){
                ...
                ResphN -=z[ion]*sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cbath[ion])/2.0*(z[ion]*phifast[phi_index(x,y,comp,Nx)])*dtf;
            }
            ierr = VecSetValue(Res,phi_index(x,y,comp,Nx),ResphN,INSERT_VALUES); CHKERRQ(ierr);
        }
    }

    ierr = VecAssemblyBegin(Res);CHKERRQ(ierr);
    ierr = VecAssemblyEnd(Res);CHKERRQ(ierr);
    ierr = restore_subarray(user->state_vars->v,user->state_vars); CHKERRQ(ierr);
    ierr = VecRestoreArray(current_state,&user->state_vars->phi_fast); CHKERRQ(ierr);
    ...
    for(x=0;x<Nx;x++) {
        for(y=0;y<Ny;y++) {
            ...
            for(comp=0;comp<Nc-1;comp++) {
                ...
                //Intra-phi with Intra-phi
                ierr = MatSetValue(Jac,phi_index(x,y,comp,Nx),phi_index(x,y,comp,Nx),Avolt,INSERT_VALUES);CHKERRQ(ierr);
                ind++;
                //Intra-phi with extra-phi
                ierr = MatSetValue(Jac,phi_index(x,y,comp,Nx),phi_index(x,y,Nc-1,Nx),AvoltN,INSERT_VALUES);CHKERRQ(ierr);
                ind++;
            }
            //Extracellular terms
            comp = Nc-1;
            ...
            for(int k=0;k<Nc-1;k++) {
                ...
                //Extra-phi with Intra-phi
                ierr = MatSetValue(Jac,phi_index(x,y,comp,Nx),phi_index(x,y,k,Nx),Avolt,INSERT_VALUES);CHKERRQ(ierr);
                ind++;
            }
            //Add bath contribution
            for(ion=0;ion<Ni;ion++){

                AvoltN -=z[ion]*sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cbath[ion])*z[ion]/2.0*dtf;
            }
            //extra-phi with extra-phi
            ierr = MatSetValue(Jac,phi_index(x,y,comp,Nx),phi_index(x,y,comp,Nx),AvoltN,INSERT_VALUES);CHKERRQ(ierr);
            ind++;
