        ...
        for(y=0;y<Ny;y++) {
            ...
            for(ion=0;ion<Ni;ion++) {
                for(comp=0;comp<Nc-1;comp++) {
                    ...
                    Resc += Rcvx - RcvxRight + Rcvy - RcvyUp;
...
//                Resc += Rcvx - RcvxRight + Rcvy - RcvyUp + flux->mflux[c_index(x,y,comp,ion,Nx)]*dt;
                ...
                Resc -= sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cfast[c_index(x,y,comp,ion,Nx)]+cbath[ion])/2.0
                        *(log(c[c_index(x,y,comp,ion,Nx)]+cfast[c_index(x,y,comp,ion,Nx)])-log(cbath[ion])+z[ion]*phi[phi_index(x,y,comp,Nx)]-z[ion]*phibath)*dt;
                ierr = VecSetValue(Res,Ind_1(x,y,ion,comp,Nx),Resc,INSERT_VALUES);CHKERRQ(ierr);
...
PetscErrorCode
calc_jacobian_slow_old(SNES snes,Vec current_state, Mat A, Mat Jac,void *ctx)
{
    ...
    for(x=0;x<Nx;x++) {
        for(y=0;y<Ny;y++) {
            ...
            for(ion=0;ion<Ni;ion++) {
                ...
                Ftmpx = sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cfast[c_index(x,y,comp,ion,Nx)]+cbath[ion])/2.0;
                Ac -= Ftmpx/(c[c_index(x,y,comp,ion,Nx)]+cfast[c_index(x,y,comp,ion,Nx)])*dt;
                Aphi -= Ftmpx*z[ion]*dt;
                Avolt -= z[ion]*Ftmpx/(c[c_index(x,y,comp,ion,Nx)]+cfast[c_index(x,y,comp,ion,Nx)])*dt;
...
PetscErrorCode calc_residual_fast_old(SNES snes,Vec current_state,Vec Res,void *ctx)
{
    //Residual equation using derivative of the charge-capacitance relation
    // Volume not solved for here
    struct AppCtx * user = (struct AppCtx *) ctx;
    ...
    PetscInt ion,comp,x,y;
    PetscReal ResphN,alpNc;
    PetscReal Resph,Resc;
        ...
        for(y=0;y<Ny;y++) {
            ...
            for(ion=0;ion<Ni;ion++) {
                for(comp=0;comp<Nc-1;comp++) {
                    ...
                    Resc += flux->mflux[c_index(x,y,comp,ion,Nx)]*dtf;
                ...
                Resc -= sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cfastp[c_index(x,y,comp,ion,Nx)]+cbath[ion])/2.0
                        *(z[ion]*phifast[phi_index(x,y,comp,Nx)]-z[ion]*phibath)*dtf;
//
                ierr = VecSetValue(Res,Ind_1(x,y,ion,comp,Nx),Resc,INSERT_VALUES);CHKERRQ(ierr);
            ...
            ResphN = 0;
            for(comp=0;comp<Nc-1;comp++) {
                Resph = cm[comp]*(phifast[phi_index(x,y,comp,Nx)]-phifast[phi_index(x,y,Nc-1,Nx)])
                        -cm[comp]*(phifastp[phi_index(x,y,comp,Nx)]-phifastp[phi_index(x,y,Nc-1,Nx)]);
                for(ion=0;ion<Ni;ion++){
                    //Ion channel
...
PetscErrorCode
calc_jacobian_fast_old(SNES snes,Vec current_state, Mat A, Mat Jac,void *ctx)
{
    //Jacobian equation using derivative of the charge-capacitance relation
    // Alpha is not solved here
    struct AppCtx * user = (struct AppCtx *) ctx;
    ...
    PetscReal Avolt,AvoltN,Ac,Aphi,Ftmpx;
    ...
    for(x=0;x<Nx;x++) {
        for(y=0;y<Ny;y++) {
            ...
            for(ion=0;ion<Ni;ion++) {
                ...
                //Add bath contributions
                Ftmpx = sqrt(pow(Dcb[c_index(x,y,comp,ion,Nx)*2],2)+pow(Dcb[c_index(x,y,comp,ion,Nx)*2+1],2))*(cp[c_index(x,y,comp,ion,Nx)]+cfastp[c_index(x,y,comp,ion,Nx)]+cbath[ion])/2.0;
                Aphi += Ftmpx*z[ion]*dtf;
    ...
    PetscReal norm;
    MatNorm(Jac,NORM_1,&norm);
    printf("%.10e\n",norm);
    KSPSolve(user->fast_slvr->ksp,user->state_vars->v_fast,user->fast_slvr->Res);
